...
# --- 3) Despike (vecinos + MAD) + Savitzky ---
...
def suavizar_resultado(y, despike_window=25, despike_k=3.5, sg_window=31, sg_poly=3):
    ...
    # asegurar ventana impar y válida
...
# --- 6) Restar blanco, suavizar y GUARDAR ---
curvas = []
...
for p in medidas:
    ...
    # suavizar DESPUÉS
    y_corr_smooth = suavizar_resultado(
        y_corr_raw,
        despike_window=25,
        despike_k=3.5,
        sg_window=31,
    ...
    # ✅ GUARDAR por archivo (dos columnas)
    df_out = pd.DataFrame({
        "Wavelength": x_m,
    ...
    # para graficar/ordenar
    curvas.append({"nombre": p.stem, "x": x_m, "y": y_corr_smooth, "orden": y_corr_smooth[-1]})

print("✅ Archivos guardados en:", SALIDA)
print("Ejemplo de archivo:", (SALIDA / f"{medidas[0].stem}_corr_smooth.csv").name)

# --- 7) Graficar todas (suavizadas) ---
...
plt.title("Medidas (medida - blanco) y luego suavizado")
plt.grid(True)
